      ...
      <form onSubmit={handleSubmit} className="space-y-0">
        ...
        <FieldGroup title="Contrato">
          <div className="grid grid-cols-2 gap-3 mt-1">
            ...
            <Input label="Qtd. Total de Parcelas *" type="number" value={form.qtd_parcelas} onChange={e => set('qtd_parcelas', e.target.value)} placeholder="Ex: 12" min={1} max={520} required />
            <Input label="Valor Total da Parcela (R$) *" type="number" value={form.valor_parcela} onChange={e => set('valor_parcela', e.target.value)} placeholder="3360.00" step="0.01" min={0} required />
            ...
            <Select label="Banco *" value={form.banco} onChange={e => set('banco', e.target.value)} required>
              <option value="">Selecione o banco…</option>
              {BANCOS.filter(b => b).map(b => <option key={b} value={b}>{b}</option>)}
            </Select>
          </div>
        </FieldGroup>
