...
const gerarParcelas = (db, cliente) => {
  ...
  const run = db.transaction((c) => {
    for (let i = 1; i <= c.qtd_parcelas; i++) {
      const vlr_parcela = Math.round((c.valor_parcela / c.qtd_parcelas) * 100) / 100;
      insert.run(uid(), c.id, i, addDays(c.data_contrato, (i - 1) * 7), vlr_parcela);
    }
  });
  run(cliente);
};
...
router.post('/', authorize('admin', 'cobranca', 'atendimento'), (req, res) => {
  ...
  if (!BANCOS_VALIDOS.includes(bnc))      return res.status(400).json({ erro: 'Banco inválido.' });
...
router.put('/:id', authorize('admin', 'cobranca', 'atendimento'), (req, res) => {
  ...
  if (!BANCOS_VALIDOS.includes(bnc)) {
    return res.status(400).json({ erro: 'Banco inválido.' });
  }
